    ...
    pool.getConnection(function onConnection(poolError, connection) {
      ...
      pool.run(function(connection, callback) {
        ...
        pool.parallelQuery([{
          ...
          {
            sql:  "select rmtid as locid, max(timedate) last_success, case when max(timedate)<sysdate-3 then 'yes' else 'no' end as investigation_needed from calibration where upper(fc4)='SUCCESS' and rmtid in (select rmtid from rmtdescription where state='A') group by rmtid order by rmtid",
          }
        ...
        results[3].forEach(function(element) {
          weather.push({
            ...
            TOTAL: element.TOTAL
                  ...
                  var calHead = Object.keys( results[5][0]);
